...
import { SCHOOL_CALENDAR_WINDOW_DAYS } from "@/lib/family";
import { createAdminClient } from "@/lib/supabase-admin";

/* A school's term calendar, linked as an ICS feed.
 *
 * The feed is fetched and parsed here and the occurrences are cached in
 * `school_calendar_events` — the cache has no insert policy, so every write
 * goes through the admin client, the same arrangement `reminders` uses.
 *
...
export const MAX_EVENTS_PER_SCHOOL = 200;
 ...
 * `webcal://` scheme every school website hands out rewritten to it. Returns
 * a sentence for the household on anything else.
 */
 ...
 * network, not a school). The address behind the name is checked separately,
 * at fetch time.
 */
 ...
 */
async function fetchCalendar(startUrl: string): Promise<string> {
...
export function parseCalendar(
  ics: string,
  now: Date = new Date(),
  windowDays: number = SCHOOL_CALENDAR_WINDOW_DAYS
): ParsedCalendar {
...
export async function syncSchoolCalendar(
  ...
  try {
    parsed = parseCalendar(await fetchCalendar(normalised.url), now);
  } catch (error) {
    const message =
